...
export class ValidationMsgDirective implements OnInit {
  ...
  createRequireDom() {
    let labelDom = this.el.nativeElement.parentNode.parentNode.childNodes[1];
    let requireInfo = document.createElement('span');
    requireInfo.classList.add('text-danger');
    requireInfo.innerText = '*';
    labelDom.appendChild(requireInfo);
    // console.log(labelDom.childNodes);
